...
from jms_consumer import get_jms_consumer
from jms_handlers import QUEUE_HANDLERS
...
app = FastAPI(
    ...
    description="Система с JWT-авторизацией и разграничением ролей (RBAC) + JMS интеграция",
    version="2.0.0",
...
# Запуск JMS Consumer при старте приложения
@app.on_event("startup")
async def startup_jms_consumer():
    """Запуск JMS consumer при старте FastAPI"""
    try:
        consumer = get_jms_consumer()
        consumer.start(QUEUE_HANDLERS)
        print(" JMS Consumer успешно запущен")
        print(" Очереди и топики:")
        print("   - /queue/customer.created.queue (обработка создания)")
        print("   - /queue/customer.updated.queue (обработка обновления)")
        print("   - /queue/customer.deleted.queue (обработка удаления)")
        print("   - /topic/customer.events.topic (события)")
    except Exception as e:
        print(f" Ошибка запуска JMS Consumer: {e}")

@app.on_event("shutdown")
async def shutdown_jms_consumer():
    """Остановка JMS consumer при завершении"""
    try:
        consumer = get_jms_consumer()
        consumer.stop()
        print(" JMS Consumer остановлен")
    except Exception as e:
        print(f" Ошибка остановки JMS Consumer: {e}")
